...
return (
  <div className="bg-gray-900 min-h-screen justify-center text-white">
    ...
    <div className="flex flex-row items-center justify-end p-4 space-x-2"> {/* Added spacing */}
...
<div className="flex flex-col items-center">
  <button 
    onClick={() => {
      ...
      }, 100); // Small delay to ensure the chatbot is rendered before scrolling
    }} 
    className="h-12 md:h-16 px-3 md:px-4 bg-purple-500 text-white rounded hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-opacity-50 flex items-center"
  >
    Go to Chatbot
  </button>
</div>
...
<img 
  ...
  alt="Chatbot"
  className="h-12 md:h-16 w-12 md:w-16 rounded-lg shadow-lg cursor-pointer hover:opacity-75"
  onClick={() => {
    toggleChatbot(); 
    setTimeout(() => {
      window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
    }, 100); // Small delay to ensure the chatbot is rendered before scrolling
  }} // Toggle the chatbot visibility on click
/>
</div>
